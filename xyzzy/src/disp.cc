...
static inline const glyph_info &
glyph_info_of_unichar (Char cc)
{
  return app.text_font.get_glyph_info (cc);
}

static inline glyph_t
glyph_of_attr_unichar (attr_t attr, Char cc)
{
  const glyph_info &info = glyph_info_of_unichar (cc);
  return glyph_of ((attr & ~GLYPH_CHARSET_MASK) | (info.font_index << GLYPH_CHARSET_SHIFT_BIT), info.glyph_index);
}

static inline glyph_t
glyph_of_unichar (Char cc)
{
  return glyph_of_attr_unichar (0, cc);
}

int char_width (Char cc) { return app.text_font.char_width (cc); }
...
paint_ascii_chars (HDC hdc, int x, int y, int flags, const RECT &r,
                   const TCHAR *string, int len, const INT *padding)
{
  const FontObject &f = app.text_font.font (FONT_ASCII);
...
paint_full_width_chars (HDC hdc, int x, int y, int flags, const RECT &r,
                        const TCHAR *string, int len, const FontObject &f)
{
  HGDIOBJ of = SelectObject (hdc, f);
...
paint_chars_lucida (HDC hdc, int x, int y, int flags, const RECT &r,
                    const TCHAR *string, int len, const INT *padding, int c)
{
...
paint_chars (HDC hdc, int x, int y, int flags, const RECT &r,
             attr_t charset, const TCHAR *string, int len, const INT *padding)
{
#ifdef UNICODE
  const FontObject &f = app.text_font.font (charset >> GLYPH_CHARSET_SHIFT_BIT);
  HGDIOBJ of = SelectObject (hdc, f);
  ExtTextOut (hdc, x + f.offset ().x, y + f.offset ().y, flags | ETO_GLYPH_INDEX,
...
Window::paint_glyphs (HDC hdc, HDC hdcmem, const glyph_t *gstart, const glyph_t *g,
                      const glyph_t *ge, TCHAR *buf, INT *padding,
                      int x, int y, int yoffset) const
#else
void
Window::paint_glyphs (HDC hdc, HDC hdcmem, const glyph_t *gstart, const glyph_t *g,
                      const glyph_t *ge, TCHAR *buf, const INT *padding,
                      int x, int y, int yoffset) const
#endif
{
  RECT r;
  ...
  while (g < ge)
    {
      ...
      *be++ = glyph_char (c);
#ifdef UNICODE
      *pe++ = app.text_font.cell ().cx;
#endif
      a &= GLYPH_COLOR_MASK | GLYPH_CHARSET_MASK;
      while (g < ge && (glyph_attr (*g) & (GLYPH_COLOR_MASK | GLYPH_CHARSET_MASK)) == a)
        {
          asum |= glyph_attr (*g);
#ifdef UNICODE
          if (glyph_trail_p (*g))
            pe[-1] += app.text_font.cell ().cx;
          else
            {
              *be++ = glyph_char (*g);
              *pe++ = app.text_font.cell ().cx;
      ...
      r.left = r.right;
#ifdef UNICODE
      r.right += (g - g0) * app.text_font.cell ().cx;
#else
          ...
          for (; b < be; b++, x += app.text_font.cell ().cx)
            {
              ...
              BitBlt (hdc, x, r.top, w, app.text_font.cell ().cy,
                      hdcmem, app.text_font.cell ().cx * _TUCHAR (*b), yoffset, SRCCOPY);
            }
        }
      else
        paint_chars (hdc, r.left + (b - buf) * app.text_font.cell ().cx, y,
                     ETO_OPAQUE | ETO_CLIPPED, r, GLYPH_CHARSET (a), b, be - b, padding);
  ...
  if (asum & GLYPH_BOLD)
    {
      ...
      g = gfrom;
      if (g > gstart && glyph_attr (g[-1]) & GLYPH_BOLD)
        {
          g--;
          if (glyph_trail_p (*g))
            g--;
      ...
      while (1)
        {
          for (; g < ge && !(glyph_attr (*g) & GLYPH_BOLD); g++)
            ;
          if (g == ge)
            break;
          TCHAR *be = buf;
          ...
          *pe++ = app.text_font.cell().cx;
#endif
          attr_t a = glyph_attr (c0) & (GLYPH_FORE_COLOR_MASK | GLYPH_BITMAP_BIT
                                        | GLYPH_CHARSET_MASK | GLYPH_BOLD);
          for (; g < ge && (glyph_attr (*g) & (GLYPH_FORE_COLOR_MASK | GLYPH_BITMAP_BIT
                                               | GLYPH_CHARSET_MASK | GLYPH_BOLD)) == a; g++)
#ifdef UNICODE
            {
              if (glyph_trail_p (*g))
                pe[-1] += app.text_font.cell ().cx;
          ...
          if (a & GLYPH_BITMAP_BIT)
            ...
            paint_chars (hdc, r.left, y, ETO_CLIPPED, r, GLYPH_CHARSET (a),
                         buf, be - buf, padding);
  ...
  if (asum & (!yoffset ? GLYPH_UNDERLINE | GLYPH_STRIKEOUT : GLYPH_UNDERLINE))
    {
      g = gfrom;
      while (1)
        {
          for (; g < ge && !(glyph_attr (*g) & (GLYPH_UNDERLINE | GLYPH_STRIKEOUT)); g++)
            ;
          if (g == ge)
            break;
          const glyph_t *g0 = g;
          glyph_t c = *g++;
          attr_t a = glyph_attr (c) & (GLYPH_FORE_COLOR_MASK | GLYPH_UNDERLINE | GLYPH_STRIKEOUT);
          for (; g < ge && (glyph_attr (*g) & (GLYPH_FORE_COLOR_MASK | GLYPH_UNDERLINE | GLYPH_STRIKEOUT)) == a; g++)
            ;
          ...
          if (a & GLYPH_UNDERLINE)
            {
              r.bottom = y + app.text_font.size ().cy;
              r.top = r.bottom - app.text_font.line_width ();
              ExtTextOut (hdc, r.left, r.top, ETO_OPAQUE, &r, _T(""), 0, 0);
            }
          if (!yoffset && a & GLYPH_STRIKEOUT)
            {
...
Window::paint_line (HDC hdc, HDC hdcmem, glyph_data *ogd, const glyph_data *ngd,
                    TCHAR *buf, int y, INT *padding) const
#else
void
Window::paint_line (HDC hdc, HDC hdcmem, glyph_data *ogd, const glyph_data *ngd,
                    TCHAR *buf, int y, const INT *padding) const
#endif
{
  const glyph_t *n = ngd->gd_cc, *ne = n + ngd->gd_len;
  glyph_t *o = ogd->gd_cc, *oe = o + ogd->gd_len;

  for (; n < ne && o < oe && glyph_equal_p (*n, *o); n++, o++)
    ;
  if (n == ne && o == oe)
    return;
  if (glyph_trail_p (*n))
    n--, o--;
  if (o > ogd->gd_cc && glyph_attr (o[-1]) & GLYPH_BOLD)
    {
      n--, o--;
      if (glyph_trail_p (*n))
        n--, o--;
  ...
  for (n = ne, o = oe; n > nfd && o > ofd && glyph_equal_p (n[-1], o[-1]); n--, o--)
    ;
...
#endif
      TCHAR *buf = (TCHAR *)alloca ((w_ch_max.cx + 3) * sizeof TCHAR);
...
static inline glyph_t *
glyph_unichar (glyph_t *g, Char cc, int f, int flags, const glyph_info &info)
{
  if (cc == _T('\\') && app.text_font.use_backsl_p ())
    *g++ = glyph_of_value (f | GLYPH_BM_BACKSL);
  else if (flags & Window::WF_HALFSPC && cc == _T(' '))  // U+0020
    *g++ = glyph_of_value ((f & ~GLYPH_TEXT_MASK) | GLYPH_CTRL | GLYPH_BM_HALFSPC);
  else if (flags & Window::WF_FULLSPC && cc == _T('Å@')) // U+3000
    {
...
glyph_dbchar (glyph_t *g, Char cc, int f, int flags)
{
  ...
  switch (code_charset (cc))
    ...
    default:
      if (flags & Window::WF_FULLSPC && cc == 0x8140U)
      ...
      else
        {
          ...
          if (!c2 || !SJISP (c1))
            goto undef_char;
          *g++ = glyph_of (f | GLYPH_LEAD | GLYPH_CHARSET_JISX0208, c1);
          *g++ = glyph_of (f | GLYPH_TRAIL | GLYPH_CHARSET_JISX0208, c2);
        }
      break;
    ...
    output:
      ...
      c1 = u_char (cc >> 8);
      c2 = u_char (cc & 0xff);
      *g++ = glyph_of (f | GLYPH_LEAD, c1);
      *g++ = glyph_of (f | GLYPH_TRAIL, c2);
      break;

    undef_char:
      *g++ = glyph_of_value (f | GLYPH_LEAD | GLYPH_BM_WBLANK1);
      *g++ = glyph_of_value (f | GLYPH_TRAIL | GLYPH_BM_WBLANK2);
      break;
    }
  return g;
}
...
glyph_sbchar (glyph_t *g, Char cc, int f, int flags)
{
  ...
  switch (ccs)
    ...
    case ccs_iso8859_2:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_1_2, cc & 255);
      break;
    ...
    case ccs_iso8859_4:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_3_4, cc & 255);
      break;

    case ccs_iso8859_5:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_5, cc & 127);
      break;

    case ccs_iso8859_7:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_7, cc & 127);
      break;
    ...
    case ccs_iso8859_10:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_9_10, cc & 255);
      break;

    case ccs_iso8859_13:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_ISO8859_13, cc & 255);
      break;

    case ccs_jisx0212:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_JISX0212_HALF,
    ...
    case ccs_jisx0201_kana:
      if (SJISP (cc))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_JISX0201_KANA, cc);
      break;

    case ccs_georgian:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_GEORGIAN, cc & 127);
      break;

    case ccs_ipa:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_IPA, cc & 127);
      break;

    case ccs_smlcdm:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | GLYPH_CHARSET_SMLCDM, cc & 255);
      break;

    case ccs_usascii:
      if (app.text_font.use_backsl_p () && cc == _T('\\'))
        *g++ = glyph_of_value (f | GLYPH_BM_BACKSL);
      else if (flags & Window::WF_HALFSPC && cc == _T(' '))
        *g++ = glyph_of_value ((f & ~GLYPH_TEXT_MASK) | GLYPH_CTRL | GLYPH_BM_HALFSPC);
      else
        *g++ = glyph_of (f, cc);
      break;
    ...
    case ccs_ulatin:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | (GLYPH_CHARSET_ULATIN1
    ...
    case ccs_ujp:
      if (i2w (cc) == ucs2_t (-1))
        goto bad_char;
      *g++ = glyph_of (f | (GLYPH_CHARSET_UJP_H1
                     ...
                     regexp_kwd &re_kwd) const
{
  ...
  if (wflags & WF_LINE_NUMBER)
    {
      attr_t f = (vlinenum == w_last_mark_linenum
                  ? (GLYPH_REVERSED | GLYPH_LINENUM)
                  : GLYPH_LINENUM);
      if (plinenum != -1 && point.p_point && point.prevch () != '\n')
        {
#ifdef UNICODE
          glyph_t c = glyph_of_attr_unichar (f, _T(' '));
#else
          glyph_t c = glyph_of (f, _T(' '));
#endif
          for (glyph_t *e = min (ge, g + LINENUM_COLUMNS); g < e; g++)
            *g = c;
          if (g < ge)
            *g++ = glyph_of_value (GLYPH_LINENUM | GLYPH_BM_SEP);
        }
      else
        {
          if (plinenum == -1)
          ...
          if (w_ch_max.cx >= LINENUM_COLUMNS + 1)
            {
              ...
              do
                {
#ifdef UNICODE
                  *--p = glyph_of_attr_unichar (f, plinenum % 10 + _T('0'));
#else
                  *--p = glyph_of (f, plinenum % 10 + _T('0'));
#endif
                  plinenum /= 10;
                }
              while (p > g && plinenum);
          ...
          else
            {
              ...
              for (int i = 0; i < n; i++)
#ifdef UNICODE
                *g++ = glyph_of_attr_unichar (f, buf[i]);
#else
                *g++ = glyph_of (f, buf[i]);
#endif
              if (g < ge)
...
#ifdef UNICODE
        g[-2] = glyph_of_attr_unichar (tprop->t_attrib & ~TEXTPROP_EXTEND_EOL_BIT, tprop->t_attrib & 0xff);
#else
        g[-2] = glyph_of (tprop->t_attrib & ~TEXTPROP_EXTEND_EOL_BIT, tprop->t_attrib & 0xff);
#endif
    }
  ...
  if (w_bufp->b_prompt_columns)
    {
      ...
      if (vlinenum == 1)
        {
          for (const TCHAR *u = w_bufp->b_prompt_arg; *u && g < ge;)
#ifdef UNICODE
            *g++ = glyph_of_unichar (*u++);
#else
            *g++ = glyph_of_char (*u++);
#endif
              ...
              if (cc < _T(' '))
                {
                  if (g + 1 == ge)
                    break;
                  *g++ = glyph_of (GLYPH_CTRL, _T('^'));
                  *g++ = glyph_of (GLYPH_CTRL, cc + _T('@'));
                }
              else if (cc == CC_DEL)
                {
                  if (g + 1 == ge)
                  ...
                  if (g < e && e < ge)
                    {
                      ...
                      for (; g < e; g++)
                        *g = glyph_of_char (_T(' '));
#endif
                      *g++ = glyph_of_value ((GLYPH_CTRL | GLYPH_BM_FOLD_SEP0) + (vlinenum & 1));
                    }
                }
            }
          *g = glyph_zero ();
          ...
          if (!cp->c_next)
            {
              ...
              if (wflags & WF_EOF
                  && (!hide || w_bufp->b_contents.p2 == w_bufp->b_nchars))
                {
                  ...
                  for (int i = 0; i < n; i++)
#ifdef UNICODE
                    *g++ = glyph_of_attr_unichar (f, _T("[EOF]")[i]);
#else
                    *g++ = glyph_of (f, _T("[EOF]")[i]);
#endif
                }
              break;
            }
      ...
      if (psi)
        {
          ...
          if (g > g0 && !(glyph_attr (g[-1]) & (GLYPH_TRAIL | GLYPH_TEXTPROP_FG_BIT)))
            glyph_attr_or (g[-1], syntax_state::ss_prev_colors[osi.ss_state][psi->si.ss_state]);
      ...
      if (hide && f & GLYPH_HIDDEN)
      ...
      else
        {
          ...
          if (cc == CC_LFD)
            {
              ...
              if (wflags & WF_NEWLINE)
                ...
                *g++ = glyph_of_attr_unichar (f & ~GLYPH_TEXT_MASK, _T(' '));
              eol = 1;
              break;
            }
          else if (cc == CC_TAB)
          ...
          else
            {
              ...
              if (cc < _T(' ') || cc == CC_DEL)
                ...
                {
                  g = glyph_unichar(g, cc, f, wflags, info);
                }
            }
#else /* UNICODE */
          if (cc < _T(' '))
              ...
              if (cc == CC_LFD)
                {
                  ...
                  if (wflags & WF_NEWLINE)
                    ...
                    *g++ = glyph_of (f & ~GLYPH_TEXT_MASK, _T(' '));
                  eol = 1;
                  break;
                }
              else if (cc == CC_TAB)
              ...
              else
                {
                  ...
                  *g++ = glyph_of ((f & ~GLYPH_TEXT_MASK) | GLYPH_CTRL, _T('^'));
                  *g++ = glyph_of ((f & ~GLYPH_TEXT_MASK) | GLYPH_CTRL, cc + _T('@'));
                }
            }
          else if (cc == CC_DEL)
            {
          ...
          if (f & GLYPH_SELECTED
            ...
            while (gr < g)
              glyph_attr_and (*gr++, ~GLYPH_SELECTED);
        }
    }
  ...
  glyph_t *tailg = g;
  int hiddenf = g > g0 ? glyph_attr (g[-1]) & GLYPH_HIDDEN : 0;
      ...
      if (w_bufp->b_fold_columns != Buffer::FOLD_NONE
          ...
          && !exceed && point.p_point == limit)
        {
          if (g == gfold && wflags & WF_FOLD_LINE)
            *g++ = glyph_of_value ((hiddenf
                                    ? (GLYPH_CTRL | GLYPH_BM_FOLD_SEP0)
                                    : (GLYPH_CTRL | GLYPH_BM_FOLD_MARK_SEP0))
                                   + (vlinenum & 1));
          else
#ifdef UNICODE
            *g++ = glyph_of_attr_unichar (GLYPH_CTRL | f | hiddenf, _T('<'));
#else
            *g++ = glyph_of (GLYPH_CTRL | f | hiddenf, _T('<'));
#endif
        }
      ...
      if ((revkwd & ((GLYPH_TEXTPROP_NCOLORS - 1)
              ...
              && (revkwd & (15 << GLYPH_COLOR_SHIFT_BITS)) <= GLYPH_KEYWORD3R))
        {
          for (glyph_t *p = g0; p < g; p++)
            glyph_attr_set (*p, ~REVMASK, revkwd);
#ifdef UNICODE
          glyph_t c = glyph_of_attr_unichar (revkwd | hiddenf, _T(' '));
#else
          glyph_t c = glyph_of (revkwd | hiddenf, _T(' '));
#endif
          for (; g < grev; g++)
            *g = c;
        }
...
#ifdef UNICODE
  for (; g > gd->gd_cc && glyph_equal_p (g[-1], spc); g--)
    ;
#else
  for (; g > gd->gd_cc && glyph_equal_p (g[-1], glyph_of_char (_T(' '))); g--)
    ;
#endif
  ...
  if (psi && point.p_offset < point.p_chunk->c_used && tailg > g0
      && psi->si.maybe_comment_p ())
    {
      ...
      if (syntax_state::ss_colors[psi->si.ss_state][osi.ss_state] == GLYPH_COMMENT)
        if (!(glyph_attr (tailg[-1]) & GLYPH_TEXTPROP_FG_BIT))
          glyph_attr_or (tailg[-1], GLYPH_COMMENT);
    }
...
#ifdef UNICODE
  for (; g > (*gr)->gd_cc && glyph_equal_p (g[-1], spc); g--)
    ;
#else
  for (; g > (*gr)->gd_cc && glyph_equal_p (g[-1], glyph_of_char (_T(' '))); g--)
    ;
#endif
  *g = glyph_zero ();
...
static inline void
format_point (TCHAR *b, int l, int c)
{
  _stprintf (b, _T("%10u:%-10u"), l, c + 1);
}
  ...
  if (stringp (fmt))
    {
      ...
      b0 = (TCHAR *)alloca ((l + 10) * sizeof TCHAR);
      b = b0;
      *b++ = _T(' ');
